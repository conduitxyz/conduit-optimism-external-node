...
`op-node` follows the chain tip over CL gossip from a single static peer
(`OP_NODE_P2P_STATIC`, written by `make setup`). On networks where Conduit's
EL peer is deployed, the same host serves both layers: CL gossip on 9222 and
the EL devp2p listener on 30303. There is no discovery network for external
nodes; the static peer is the only CL connectivity the node has.
...
| `UPDATE_BEDROCK_BLOCK` | Optional Ronin/Zircuit setting; when `true`, updates `genesis.json` with the network-specific migration `bedrockBlock` during `make setup` |
| `RETH_HISTORICAL_RPC` | Optional pre-bedrock historical RPC used by Ronin/Saigon/Zircuit migration networks when `UPDATE_BEDROCK_BLOCK=true` |
...
**Note:** For migration networks, set `UPDATE_BEDROCK_BLOCK=true` before running setup. This is supported for `saigon-testnet-cc58e966ql`, `ronin-mainnet-bfz9fadqzl`, `zircuit-garfield-testnet`, and `zircuit-mainnet`. If the node needs pre-bedrock historical data, set `RETH_HISTORICAL_RPC` to an RPC endpoint that can serve it and uncomment the `--rollup.historicalrpc=${RETH_HISTORICAL_RPC}` line in the applicable Compose file. Leave that line commented for other chains.
