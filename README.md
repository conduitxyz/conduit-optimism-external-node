...
**Note:** For migration networks, set `UPDATE_BEDROCK_BLOCK=true` before running setup. This is supported for `saigon-testnet-cc58e966ql`, `ronin-mainnet-bfz9fadqzl`, and `zircuit-garfield-testnet`. If the node needs pre-bedrock historical data, set `RETH_HISTORICAL_RPC` to an RPC endpoint that can serve it and uncomment the `--rollup.historicalrpc=${RETH_HISTORICAL_RPC}` line in the applicable Compose file. Leave that line commented for other chains.
